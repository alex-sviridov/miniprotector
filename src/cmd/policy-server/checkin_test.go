...
func TestRunCheckinCleanup_RemovesRecordsOlderThanRetention(t *testing.T) {
	...
	require.NoError(t, store.RecordCheckin(t.Context(), "policy-1", "stale-host", time.Now().Add(-time.Hour)))
	require.NoError(t, store.RecordCheckin(t.Context(), "policy-1", "fresh-host", time.Now()))
