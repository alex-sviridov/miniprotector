...
- `api-server`: `?type=` query param on `GET /policies`; `hostname`/`port`/`config` on `policyDTO`;
...
- Client filters (hostnames/labels) editing in the storage modal. Storage policies created through
  this UI get empty `client_filters`, which `PolicyBase.Matches` already treats as "matches any
  node" — the same default a backup policy gets if its hostnames list is left empty. Editing filters
  for storage policies can be added later if needed.
...
- Anything about a storage server actually consuming `hostname`/`port`/`config` to run. This is
  still policy authoring only, same boundary the backend design drew.
...
- `policyDTO` gains `Hostname string`, `Port int32`, `Config string` fields, populated from
  `p.GetHostname()`/`p.GetPort()`/`p.GetConfig()` in `toPolicyDTO` — empty/zero for backup policies,
  same additive convention as the proto.
- New `storagePolicyInput` struct: `Name string`, `ClientFilters clientFiltersDTO`,
  `Hostname string`, `Port int32`, `Config string`.
- New handlers `handleCreateStoragePolicy` / `handleUpdateStoragePolicy`, parallel to the existing
  ...
  with `Hostname`/`Port`/`Config` set and `Type: "storage"` (create only; update has no type field,
...
`DataTable` (columns: Name, Hostname, Port, Storage Type, actions). Row's Name links/click opens the
edit modal for that row (rather than navigating to a detail route); "New Storage Policy" opens the
same modal empty. Delete button per row, same `window.confirm` pattern as `PoliciesListView`.

Storage type displayed in the table is derived client-side by parsing `row.config` and reading its
`type` field (falls back to `—` if unparseable/absent).
...
On mount with a non-null `policy` prop, the form is pre-filled from `policy.hostname`/`policy.port`
and `JSON.parse(policy.config)` (`type`/`path`). On submit, builds
`config: JSON.stringify({ type: storageType, path })` and calls `storagePolicies.create`/`update`
via the parent view (the modal emits `save` with the built payload; `StorageView` owns the store
calls and closes the modal on success, mirroring how `PolicyFormView` owns `policies.create`/
`update` today). `client_filters` sent as `{ hostnames: [], labels: {} }` — not user-editable here
(see Out of scope).

Client-side validation before emitting `save`: name/hostname/path non-empty, port in range. Server
-side validation (`StoragePolicy.Validate()`) is the authority; client checks just avoid an obviously
bad round-trip.
...
2. User fills name/hostname/port/type/path, submits → modal validates client-side, emits `save`
   with `{ name, hostname, port, config, client_filters }`.
...
6. `api-server` returns `policyDTO` (now including hostname/port/config) with `201`.
...
- **`api-server`**: `handleListPolicies` passes `?type=` through to `ListPoliciesRequest.Type`;
  `toPolicyDTO` surfaces hostname/port/config; `handleCreateStoragePolicy`/
...
- **Web UI**: `storagePolicies` store actions (fetchAll/create/update/remove) against a mocked
  `apiFetch`, same pattern as `stores/policies.spec.js`; `StorageView` renders the list, opens the
  modal on "New" and on row click, calls the right store action on save; `StorageEditModal` pre-fills
  from an edit `policy` prop, round-trips `config` JSON correctly, validates required fields
  client-side before emitting `save`.
