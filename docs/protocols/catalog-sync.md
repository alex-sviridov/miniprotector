...
`source_node`; any caller may query across hosts via the `source_host` filter.
...
- `source_host` — exact match against the same CA-verified hostname `SyncFileVersions` persists as
  `source_node`; empty matches every host.
...
- `path`, `size`, `mode`, `owner`, `group`, and `mod_time` are decoded server-side from the same
  ...
  interpret that blob's contents rather than just persisting it.
